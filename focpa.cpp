...
int first_order(Config & conf)
{
  ...
  for (int bn = 0; bn < conf.key_size; bn++){
    ...
    for (int bit=0; bit >= 0 && bit < bitsperbyte; bit=(bit!=-1)?bit+1:bit) {
      ...
      res = construct_guess (&fin_conf.mat_args->guess, conf.algo, conf.guesses, conf.ciphertexts, conf.n_file_guess, bn, conf.round, conf.des_switch, conf.sbox, conf.total_n_keys, bit);
      if (res < 0) {
