...
*/
  ...
  for (int bn = 0; bn < conf.key_size; bn++){
     ...
     * key bytes attacked.
     */
    res = construct_guess (&fin_conf.mat_args->guess, conf.algo, conf.guesses, conf.ciphertexts, conf.n_file_guess, bn, conf.round, conf.des_switch, conf.sbox, conf.total_n_keys, -1);
    if (res < 0) {
