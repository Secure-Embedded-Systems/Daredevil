...
int construct_guess (TypeGuess ***guess, uint32_t alg, Matrix *m, Matrix * ct, uint32_t n_m, uint32_t bytenum, uint32_t R, uint32_t des_switch, uint16_t * sbox, uint32_t n_keys, int8_t bit) {
  int ret;

  switch (alg) {
    case ALG_AES:
      ret = construct_guess_AES (guess, m, ct, n_m, bytenum, R, sbox, n_keys, bit);
      if (ret < 0) return -1;
      break;
    case ALG_DES:
...
template int construct_guess (uint8_t ***guess, uint32_t alg, Matrix *m, Matrix *ct, uint32_t n_m, uint32_t bytenum, uint32_t R, uint32_t des_switch, uint16_t * sbox, uint32_t n_keys, int8_t bit);
